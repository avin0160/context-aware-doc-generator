...
def _detect_basic_extraction(doc: str) -> bool:
    """
    Detect if documentation is just basic code extraction (listings) vs real AI-generated docs.
    
    Basic extraction indicators:
    1. Contains "Generated by Context-Aware Documentation Generator" (fallback signature)
    2. High ratio of function/class signatures to explanatory text
    3. Very few sentences that actually explain functionality
    4. Just headers + bullet lists with no prose
    
    Returns:
        True if this appears to be basic extraction, False if real documentation
    """
    import re
    
    # Check for fallback signature
    if "Generated by Context-Aware Documentation Generator" in doc:
        return True
    
    # Check for telltale signs of basic extraction
    basic_indicators = [
    ...
    indicator_count = sum(1 for indicator in basic_indicators if indicator in doc)
    if indicator_count >= 3:
        return True
    
    # Count actual explanatory sentences vs listings
    lines = doc.split('\n')
    ...
    code_lines = sum(1 for line in lines if line.strip().startswith(('def ', 'class ', 'async def ', 'function ', '```')))
    
    # Count explanatory prose (sentences with 10+ words that aren't headers/bullets/code)
    prose_sentences = 0
    for line in lines:
        stripped = line.strip()
        if not stripped:
        ...
        if len(words) >= 10 and any(c in stripped for c in '.!?'):
            prose_sentences += 1
    
    # If very few prose sentences relative to structure, it's basic extraction
    total_content_lines = header_lines + bullet_lines + code_lines + prose_sentences
    if total_content_lines > 0:
        prose_ratio = prose_sentences / total_content_lines
        if prose_ratio < 0.15:  # Less than 15% is actual explanation
            return True
    
    # If doc is just a list of function names without descriptions
    func_listings = len(re.findall(r'def \w+\([^)]*\)', doc))
    func_descriptions = len(re.findall(r'(Args:|Parameters:|Returns:|Description:)', doc, re.IGNORECASE))
    
    if func_listings > 3 and func_descriptions == 0:
        return True
