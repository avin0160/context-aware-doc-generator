...
def _detect_basic_extraction(doc: str) -> bool:
    ...
    CRITICAL: Must NOT flag real AI documentation OR tautological documentation with descriptions!
    
    Basic extraction indicators (DEFINITIVE):
    1. Contains explicit fallback signatures (WITHOUT tautological descriptions)
    ...
    # === TAUTOLOGICAL DOCS ARE NOT BASIC EXTRACTION ===
    # If it has tautological descriptions, it's a valid (if simple) documentation
    if "tautological analysis" in doc:
        return False
    
    # === DEFINITIVE FALLBACK SIGNATURES ===
    # If these exact strings appear WITHOUT tautological marker, it's basic extraction
    definitive_signatures = [
        "*Generated by Context-Aware Documentation Generator*",  # Old basic signature
    ]
    # Only flag if it ALSO lacks Description sections
    for sig in definitive_signatures:
        if sig in doc:
            # Check if it has actual descriptions
            if "**Description:**" not in doc and "Description:" not in doc:
                return True
    ...
    if word_count < 500:
        ...
        # Only flag as basic if it has listing patterns AND no descriptions
        if listing_count >= 2 and "**Description:**" not in doc:
            return True
    ...
    if re.search(r'Files analyzed:\s*\d+', doc) and re.search(r'Functions found:\s*\d+', doc):
        ...
        # Only flag if it lacks actual descriptions
        if func_listings + class_listings > 3 and after_words < 300 and "**Description:**" not in doc:
            return True
